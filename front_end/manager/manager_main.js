  ...
  //   const customer = await fetch(`http://localhost:8080/managerbyid?id=${window.sessionStorage.getItem("id")}`, {
   ...
   let noOfBookings=-1;
    custdata.data.map((manager)=>{
      ...
      <td><button id="pending" class="pen accept">${manager.payment}</button></td>
      ...
      `
      table.innerHTML += tr
      if(manager.payment==="pending"){
      let pen_btn=  document.querySelectorAll("#pending");
      pen_btn[noOfBookings].classList.remove("accept");

      }
      else{
        console.log("hiii");
        let pen_btn=  document.querySelectorAll("#pending");
        pen_btn[noOfBookings].classList.add("accept");
      }
...
//==========================================================================
function click(req_accpet_btn, i){
  let booking_id=document.getElementsByClassName("booking_id")[i].innerHTML
  

  req_accpet_btn.addEventListener("click", async ()=>{
    console.log(req_accpet_btn);
    console.log("hii");
    let msg=req_accpet_btn.innerHTML;
    if(msg==="pending"){
      req_accpet_btn.classList.remove("accept")
      // req_accpet_btn.style.color="blue"
      // req_accpet_btn.style.backgroundColor="rgb(0, 255, 0)"
      // req_accpet_btn.innerHTML="Requist Sent";
       booking_id=document.getElementsByClassName("booking_id")[i].innerHTML
      ...
      req_accpet_btn.classList.remove("pen")
     
     
      // req_accpet_btn.style.color="white"
            req_accpet_btn.style.backgroundColor="red"
            req_accpet_btn.innerHTML="Pending"
            //=================================================================================================
            booking_id=document.getElementsByClassName("booking_id")[i].innerHTML
            let msg="pending"
            try {
...
async function building(){
  ...
  floors.map((flr)=>{
    ...
    //           }
      ...
      <label class="capacity">capacity = <span id="capacity1">${sp.capacity}</span></label> 
      <br>
  ...
  function edit(edit_btn, i){
    edit_btn.addEventListener("click", (e)=>{
      ...
      let capacity=document.querySelectorAll("#capacity1")[i].innerHTML
...
main_button.addEventListener("click",()=>{
  window.open("http://127.0.0.1:5500/front_end/front_end/manager/create_workspace.html")
})
