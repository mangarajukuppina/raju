...
async function customerById(){
  try{
    const client = await fetch(`http://localhost:8080/clientbyemail?client_email=${localStorage.getItem('client_email')}`,{
      method: 'GET',
      headers: {
    ...
    pincode.innerHTML = clientdata.data.address.pincode; 
      ...
      window.alert(clientdata.messege)
...
updatecustomer.addEventListener('click', ()=>{
  window.open("http://127.0.0.1:5500/front_end/front_end/Client/client-update.html")
...
async function workspaces(){
  ...
  const response = await fetch(`http://localhost:8080/workspaceall`, {
  ...
  if(workspace.status===302){
    // console.log(workspace.data);
    workspace.data.map((sp)=>{
      ...
      //           }
        ...
        <br></br>
        <button id="edit_btn"><a href="http://127.0.0.1:5500/front_end/images/manager/updateworkspace.html" target="blank">Book Now</a></button>
      ...
      function edit(edit_btn, i){
        edit_btn.addEventListener("click", (e)=>{
          ...
          window.open("http://127.0.0.1:5500/front_end/front_end/Client/clientBooking.html")
      ...
      // =============================================================================================================
        ...
        window.alert("buldings not available")
...
//================================================================================================================
// clientBokings
async function clientBookings(){
  let noOfBookings=-1;
  try {
    let response=await fetch(`http://localhost:8080/clientallbookings?client_id=${localStorage.getItem('client_id')}`, {
    ...
    let client_bookings=await response.json();
    // console.log(client_bookings);
    if(client_bookings.status===302){
      // console.log("hii");

  
    client_bookings.data.map((client_booking)=>{
      noOfBookings++;
      let table=document.getElementById("table")
    
      let tr = `
      <tr>
      <td class="booking_id">${client_booking.booking_id}</td>
      <td id="cost">${client_booking.cost}</td>
      <td>${client_booking.entryDate}</td>
      <td>${client_booking.exitdate}</td>
      <td><button id="pending" class="pen accept">${client_booking.payment}</button></td>
      ...
      `
      table.innerHTML += tr
      if(client_booking.payment==="pending"){
        let pen_btn=  document.querySelectorAll("#pending");
        pen_btn[noOfBookings].classList.remove("accept");
  
        }
        else if(client_booking.payment==="accepted"){
          console.log("hiii");
          let payment_btn=  document.querySelectorAll("#pending")[noOfBookings]
          // console.log(payment_btn);
          payment_btn.innerHTML="pay"
          payment_btn.style. cursor= "pointer";
          payment_btn.style.fontWeight="800"
          payment_btn.style.width="140px"
          payment_btn.style.fontSize="30px"
          payment_btn.style.color="red"
            let pen_btn=  document.querySelectorAll("#pending");
        ...
        let payment_btn=  document.querySelectorAll("#pending")[noOfBookings]
          // console.log(payment_btn);
          payment_btn.innerHTML="Booked"
            let pen_btn=  document.querySelectorAll("#pending");
            pen_btn[noOfBookings].classList.add("accept");
            console.log(payment_btn);
            payment_btn.style.backgroundColor="blue"
        }
  ...
  let payment_btn=  document.querySelectorAll("#pending")
  // console.log(payment_btn);
  for(let i=0;i<payment_btn.length;i++){
    console.log(payment_btn[i].innerHTML);
    
    if(payment_btn[i].innerHTML==="pay"){
      console.log("raju");
      paymentRequest(payment_btn[i],i);
    }
  ...
  //payment
  function paymentRequest(payment_btn, i){
    let payment_btn1=  document.querySelectorAll("#pending")[i]
    console.log(payment_btn);
    payment_btn.addEventListener("click", ()=>{
      let td_cost=document.querySelectorAll("#cost")[i]
      let cost=td_cost.innerHTML;
      let booking_id_btn=document.querySelectorAll(".booking_id")[i]
      let booking_id=booking_id_btn.innerHTML
      window.localStorage.setItem("booking_id", booking_id)
      console.log("devil");
      window.localStorage.setItem("cost", cost);
      window.open("http://127.0.0.1:5500/front_end/front_end/Client/client_payment.html")
    })
